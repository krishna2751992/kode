package gov.hhs.cms.desy.web.rest.interceptors.mock;
...
@Named
@Profile("dev")
public class MockUserInterceptor extends HandlerInterceptorAdapter {

	private final Logger log = LoggerFactory.getLogger(MockUserInterceptor.class);

	private static final String ADMIN_USER_ROLE = "cn=DESY_D_ADMIN,ou=Groups,dc=cms,dc=hhs,dc=gov:cn=DESY_D_USER,ou=Groups,dc=cms,dc=hhs,dc=gov";
	private static final String USER_ROLE = "cn=DESY_D_USER,ou=Groups,dc=cms,dc=hhs,dc=gov";
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
			throws Exception {
		log.info("********** MockUserInterceptor in DEV Profile ****************** ");
		String fName = "Will";
		String lName = "Smith";
		String email = "will-smith@disney.com";
		String uid = "NDOM";
		...
		log.info("User role/ismemberof from header, {}", ADMIN_USER_ROLE);

		this.parseRoles(ADMIN_USER_ROLE, authorities);
	...
	private void parseRoles(String roles, Set<String> authorities) {
		Set<String> roleSet = Stream.of(roles.split(REGEX)).map(elem -> new String(elem)).collect(Collectors.toList())
				.stream().filter(elem -> elem.contains(DESY)).collect(Collectors.toSet());
